...
public class PersonClient {
    ...
    public PersonResponse getPersonDetails(int id) {
        return restTemplate.getForObject("http://PERSON-USER-PROFILE-API/api/person/id?personId="+id, PersonResponse.class);
    }

    @HystrixCommand(fallbackMethod = "getPersonList")
    public List<PersonResponse> getPersons() {
        ResponseEntity<List<PersonResponse>> response = restTemplate.exchange("http://PERSON-USER-PROFILE-API/api/person/", HttpMethod.GET, null, new ParameterizedTypeReference<List<PersonResponse>>(){});
   ...
   //  return restTemplate.exchange("http://PERSON-USER-PROFILE-API/api/person/", HttpMethod.POST, null, String.class).getBody();
        String response = restTemplate.postForObject(
                "http://PERSON-USER-PROFILE-API/api/person/",personRequest,
                String.class);
        return response;
    }
    ...
    public String deletePersonById(int id) {
        restTemplate.exchange("http://PERSON-USER-PROFILE-API/api/person/?personId="+id,HttpMethod.DELETE,null,String.class).getBody().toString();
    ...
    public List<PersonResponse> filterPersonsByKey(Map<String,String> allParams) {
        ...
        for (Map.Entry<String, String> entry : allParams.entrySet()) {
            response = restTemplate.exchange("http://PERSON-USER-PROFILE-API/api/person/search?"+entry.getKey()+"="+entry.getValue(),
